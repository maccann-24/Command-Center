"""
Standalone test for backtesting/metrics.py module
Tests metrics calculation without requiring full dependency chain
"""
...
# Minimal Trade and BacktestResults definitions (duplicated to avoid dependencies)
@dataclass
class Trade:
...
def test_metrics_basic():
    ...
    assert abs(report.total_return - 15.0) < 0.01, f"Total return mismatch: expected 15.0, got {report.total_return}"
    
    print(f"✓ Win Rate: {report.win_rate:.1f}%")
    assert abs(report.win_rate - 75.0) < 0.1, f"Win rate mismatch: expected 75.0, got {report.win_rate}"
    
    print(f"✓ Trade Count: {report.trade_count}")
    assert report.trade_count == 4, f"Trade count mismatch: expected 4, got {report.trade_count}"
    ...
    # Expected drawdown: from 1200 to 1110 = 90/1200 = 7.5%
    expected_dd = ((1200.0 - 1110.0) / 1200.0) * 100.0
    print(f"  (Expected ~{expected_dd:.2f}%, got {report.max_drawdown:.2f}%)")
