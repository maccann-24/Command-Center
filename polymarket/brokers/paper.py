...
class PaperBroker(BrokerAdapter):
    ...
    def _log_execution(self, order: Order, execution: Execution) -> None:
        ...
        if not self.db_available:
            print(
                f"⚠️  Paper execution not logged (DB unavailable): {execution.order_id}"
            )
            return
        ...
        try:
            ...
            if response.data and len(response.data) > 0:
                ...
                position = Position(
                    ...
                    thesis_id=(
                        uuid.UUID(pos_data["thesis_id"])
                        if pos_data.get("thesis_id")
                        else None
                    ),
                    opened_at=(
                    ...
                    closed_at=(
                        datetime.fromisoformat(pos_data["closed_at"])
                        if pos_data.get("closed_at")
                        else None
                    ),
                    created_at=(
                        datetime.fromisoformat(pos_data["created_at"])
