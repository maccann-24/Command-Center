...
class InMemoryThesisStore:
    ...
    def get_by_market(self, market_id: str):
        ...
        return [
            t for t in self.theses if t.market_id == market_id and t.status == "active"
        ]
...
def test_run_cycle_with_execution():
    ...
    assert (
        stats["theses_actionable"] >= 1
    ), "Should have actionable thesis (conviction=0.85)"
...
def test_run_cycle_multiple_agents():
    ...
    assert (
        stats["theses_generated"] >= 3
    ), f"Should generate 3+ theses, got {stats['theses_generated']}"
...
def test_error_handling():
    ...
    assert (
        "ErrorAgent" in str(stats["errors"])
        or "agent error" in str(stats["errors"]).lower()
