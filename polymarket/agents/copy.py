...
class CopyAgent(BaseAgent):
    ...
    def update_theses(self) -> List[Thesis]:
        ...
        try:
            from database import post_analyzing_message
            trading_floor_enabled = True
        except ImportError:
            trading_floor_enabled = False

        # Post "analyzing" message to Trading Floor (even for stub)
        if trading_floor_enabled:
            post_analyzing_message(
                agent_id="copy-trader-bot",
                theme="copy-trading",
                content="⚠️ Copy trading agent not yet implemented - awaiting Polymarket leaderboard API"
            )
