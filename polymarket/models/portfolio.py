...
class Position:
    ...
    def __post_init__(self):
        ...
        if not 0.0 <= self.entry_price <= 1.0:
            raise ValueError(
                f"entry_price must be between 0.0 and 1.0, got {self.entry_price}"
            )

        if not 0.0 <= self.current_price <= 1.0:
            raise ValueError(
                f"current_price must be between 0.0 and 1.0, got {self.current_price}"
            )
    ...
    def from_dict(cls, data: dict) -> "Position":
        ...
        if (
            "thesis_id" in data
            and data["thesis_id"]
            and isinstance(data["thesis_id"], str)
        ):
