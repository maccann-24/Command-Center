...
class BacktestResults:
    """Results from a backtest run including trades and equity curve."""
...
def example_winning_strategy():
    ...
    print(
        f"  → Sharpe ratio of {report.sharpe_ratio:.2f} shows excellent risk-adjusted returns"
    )
    print(f"  → Max drawdown of {report.max_drawdown:.1f}% indicates low risk")
    print(
        f"  → Edge captured: {report.avg_edge_captured:.0f}% (outperforming expectations)"
    )
