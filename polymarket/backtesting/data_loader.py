...
def fetch_historical_markets(
    days_back: int = 90, target_count: int = 200, categories: Optional[List[str]] = None
) -> int:
    """
    ...
    try:
        ...
        for batch_num in range(max_batches):
            ...
            print(
                f"📡 Fetching batch {batch_num + 1} (offset={offset}, limit={batch_size})..."
            )
            ...
            try:
                ...
                if response.status_code != 200:
                    print(
                        f"⚠️  API returned status {response.status_code}, stopping batch fetch"
                    )
                    break
...
def parse_historical_market(
    data: Dict[str, Any], cutoff_date: datetime, target_categories: List[str]
...
def get_loaded_count() -> int:
    ...
    try:
        result = (
            supabase.table("historical_markets").select("id", count="exact").execute()
        )
...
if __name__ == "__main__":
    ...
    loaded_count = fetch_historical_markets(
        days_back=90, target_count=200, categories=["geopolitical", "crypto", "sports"]
    )
