...
def test_metrics_basic():
    ...
    assert (
        abs(report.total_return - 15.0) < 0.01
    ), f"Total return mismatch: expected 15.0, got {report.total_return}"

    print(f"✓ Win Rate: {report.win_rate:.1f}%")
    assert (
        abs(report.win_rate - 75.0) < 0.1
    ), f"Win rate mismatch: expected 75.0, got {report.win_rate}"

    print(f"✓ Trade Count: {report.trade_count}")
    assert (
        report.trade_count == 4
    ), f"Trade count mismatch: expected 4, got {report.trade_count}"
