...
def get_supabase_client() -> Client:
    ...
    if _supabase_client is None:
        # Prefer service role key for server-side writes so we can enforce strict RLS
        # (authenticated read-only) for dashboards.
...
def save_position(position: Position) -> bool:
    ...
    try:
        ...
        record_event(
            event_type=(
                "position_created" if position.status == "open" else "position_updated"
            ),
...
def test_connection() -> bool:
    ...
    try:
        ...
        result = (
            supabase.table("markets").delete().eq("id", "test_market_123").execute()
        )
