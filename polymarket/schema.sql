...
-- Time-series snapshots for charting (sparklines/equity curve)
-- ============================================================
CREATE TABLE IF NOT EXISTS portfolio_history (
    ...
    deployed_pct DECIMAL(6, 2) DEFAULT 0.00,
    daily_pnl DECIMAL(15, 2) DEFAULT 0.00,
    all_time_pnl DECIMAL(15, 2) DEFAULT 0.00
);

CREATE INDEX IF NOT EXISTS idx_portfolio_history_ts ON portfolio_history(ts DESC);
...
BEGIN
    ASSERT (SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN 
        ('markets', 'historical_markets', 'news_events', 'trader_performance', 'theses', 'portfolio', 'portfolio_history', 'positions', 'ic_memos', 'event_log')
    ) = 10, 'Schema creation failed: missing tables';
    
    RAISE NOTICE 'Schema validation passed: All 9 tables created successfully';
END $$;
