...
def test_failing_win_rate():
    ...
    assert any("signal quality" in r.lower() for r in result.recommendations), "Should recommend signal improvement"
...
def test_failing_sharpe():
    ...
    assert any("position sizes" in r.lower() for r in result.recommendations), "Should recommend smaller positions"
...
def test_failing_drawdown():
    ...
    assert any("stop loss" in r.lower() for r in result.recommendations), "Should recommend risk controls"
...
def test_failing_trade_count():
    ...
    assert any("market coverage" in r.lower() for r in result.recommendations), "Should recommend more markets"
...
def test_failing_return():
    ...
    assert result.approved == False, "Should be rejected"
    assert any("return" in f.lower() for f in result.failures), "Should flag negative return"
    assert any("revision" in r.lower() for r in result.recommendations), "Should recommend strategy revision"
...
def test_multiple_failures():
    ...
    assert result.approved == False, "Should be rejected"
    assert len(result.failures) == 5, f"Should have 5 failures, got {len(result.failures)}"
    assert len(result.recommendations) == 5, f"Should have 5 recommendations, got {len(result.recommendations)}"
