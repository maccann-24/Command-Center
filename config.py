...
def validate_config():
    ...
    if not 0.0 < RISK_PARAMS["max_position_pct"] <= 100.0:
        errors.append(f"max_position_pct must be between 0 and 100, got {RISK_PARAMS['max_position_pct']}")
    
    if not 0.0 < RISK_PARAMS["max_deployed_pct"] <= 100.0:
        errors.append(f"max_deployed_pct must be between 0 and 100, got {RISK_PARAMS['max_deployed_pct']}")
    
    # min_conviction is stored as fraction (0-1)
    if not 0.0 < RISK_PARAMS["min_conviction"] <= 1.0:
        errors.append(f"min_conviction must be between 0 and 1, got {RISK_PARAMS['min_conviction']}")
...
def print_config_summary():
    ...
    if LIVE_TEST_MODE:
        print(f"Live Test Mode:      ENABLED (max ${LIVE_TEST_LIMITS['max_trade_size_usd']:.2f} per trade)")
