...
class RiskEngine:
    ...
    def evaluate(self, thesis, portfolio) -> RiskDecision:
        ...
        if proposed_size < max_position:
            risk_notes.append(f"✓ Position size {proposed_size:.1f}% ≤ {max_position:.1f}%")
        else:
            risk_notes.append(f"✗ Position size {proposed_size:.1f}% > {max_position:.1f}%")
            failed_checks.append(f"Position size {proposed_size:.1f}% exceeds limit {max_position:.1f}%")
        ...
        if total_after_trade < max_deployed:
            risk_notes.append(f"✓ Total deployed {total_after_trade:.1f}% ≤ {max_deployed:.1f}%")
        else:
            risk_notes.append(f"✗ Total deployed {total_after_trade:.1f}% > {max_deployed:.1f}%")
            failed_checks.append(f"Total deployed {total_after_trade:.1f}% exceeds limit {max_deployed:.1f}%")
        ...
        if conviction >= min_conviction:
            ...
            risk_notes.append(f"✗ Conviction {conviction:.2f} < {min_conviction:.2f}")
            failed_checks.append(f"Conviction {conviction:.2f} below minimum {min_conviction:.2f}")
        ...
        if edge > min_edge:
            ...
            risk_notes.append(f"✗ Edge {edge:.2%} ≤ {min_edge:.2%}")
            failed_checks.append(f"Edge {edge:.2%} insufficient (minimum {min_edge:.2%})")
