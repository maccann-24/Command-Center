...
def generate_daily_memo(
    ...
    if theses:
        ...
        for thesis in theses:
            market_label = _get_attr(thesis, "market_question", "question", "market_id", default="(unknown market)")
            market_name = market_label[:50] + "..." if len(market_label) > 50 else market_label
            ...
            edge = f"{edge_val:.1%}" if isinstance(edge_val, (int, float)) else str(edge_val)
            conviction = f"{conviction_val:.2f}" if isinstance(conviction_val, (int, float)) else str(conviction_val)
    ...
    if trades:
        ...
        # Table rows
        for trade in trades:
            market_label = _get_attr(trade, "market_question", "question", "market_id", default="(unknown market)")
            market_name = market_label[:40] + "..." if len(market_label) > 40 else market_label
            ...
            thesis_id = thesis_id_raw[:8] + "..." if len(thesis_id_raw) > 8 else thesis_id_raw

            size = f"{shares:.0f}" if isinstance(shares, (int, float)) else str(shares)
            fill_price = f"${entry_price:.2f}" if isinstance(entry_price, (int, float)) else str(entry_price)
            ...
            lines.append(f"| {market_name} | {action} | {size} | {fill_price} | {thesis_id} | {outcome} |")
    ...
    if trades:
        ...
        if completed_trades:
            ...
            lines.append(f"- **Win Rate:** {win_rate:.1f}% ({winning_trades}/{total_trades})")
    ...
    lines.append("")
    lines.append("*This is an automated trading system. Not financial advice. Trade at your own risk.*")
    lines.append("")
    ...
    try:
        ...
        daily_return = None
        try:
            if getattr(portfolio, "total_value", None) not in (None, 0) and getattr(portfolio, "cash", None) is not None:
