...
def test_approved_execution():
    ...
    assert position.status == "open", f"Position status should be 'open', got {position.status}"
    assert position.thesis_id == thesis.id, "Position not linked to thesis"
    
    print(f"  ✓ Position created: {position.shares:.2f} shares @ ${position.entry_price:.4f}")
    ...
    print(f"  ✓ Cash: ${initial_cash:.2f} → ${portfolio.cash:.2f} (cost: ${expected_cost:.2f})")
    ...
    assert abs(portfolio.deployed_pct - expected_deployed) < 1.0, \
    ...
    assert order.size == 100.0, f"Order size should be $100 (10% of $1000), got ${order.size}"
    
    print(f"  ✓ Order executed: {order.side} ${order.size:.2f} @ ${order.limit_price:.4f}")
...
def test_rejected_execution():
    ...
    try:
        ...
        assert "rejected trade" in str(e).lower(), "Error message should mention rejected trade"
        assert "conviction too low" in str(e).lower(), "Error message should contain rejection reason"
    
    # ============================================================
    # VERIFY NO POSITION CREATED
    # ============================================================
    assert len(portfolio.positions) == initial_positions, "No position should be created"
    ...
    # ============================================================
    assert len(broker.executed_orders) == 0, "Broker should not have executed any orders"
...
def test_double_check_safety():
    ...
    try:
        ...
        assert "portfolio state changed" in str(e).lower() or "deployed" in str(e).lower(), \
    ...
    # VERIFY NO ORDER EXECUTED
    # ============================================================
    assert len(broker.executed_orders) == 0, "Broker should not execute when double-check fails"
    print("  ✓ No broker execution")
