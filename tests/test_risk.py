...
spec = importlib.util.spec_from_file_location('risk', os.path.join(os.path.dirname(__file__), '..', 'core', 'risk.py'))
...
def test_approved():
    ...
    portfolio = MockPortfolio(
        cash=1000.0,
        deployed_pct=40.0  # 40% deployed, 60% available
    )
    ...
    assert decision.recommended_size == 15.0, f"Expected size 15.0, got {decision.recommended_size}"
    assert "All checks passed" in decision.reason, f"Unexpected reason: {decision.reason}"
    
    # Verify: Spec requires a single success note
    assert decision.risk_notes == ["All checks passed"], f"Unexpected risk_notes: {decision.risk_notes}"
...
def test_position_too_large():
    ...
    assert decision.approved == False, f"Expected approved=False, got {decision.approved}"
    assert "Position size" in decision.reason, f"Expected 'Position size' in reason, got: {decision.reason}"
    assert decision.recommended_size == 0.0, f"Expected recommended_size=0.0, got {decision.recommended_size}"
    
    # Verify: Position size check failed
    position_check_failed = any("Position size" in note and "✗" in note for note in decision.risk_notes)
...
def test_portfolio_over_deployed():
    ...
    portfolio = MockPortfolio(
        cash=1000.0,
        deployed_pct=50.0  # 50% deployed
    )
    
    # Setup: Thesis with 15% position (total would be 65% > 60% limit)
    thesis = MockThesis(
        proposed_action={'size_pct': 15.0},  # 15% position
        conviction=0.80,
        edge=0.10
    )
    ...
    # Verify: Should be rejected
    assert decision.approved == False, f"Expected approved=False, got {decision.approved}"
    assert "deployed" in decision.reason.lower(), f"Expected 'deployed' in reason, got: {decision.reason}"
    assert decision.recommended_size == 0.0, f"Expected recommended_size=0.0, got {decision.recommended_size}"
    
    # Verify: Total deployed check failed
    deployed_check_failed = any("deployed" in note.lower() and "✗" in note for note in decision.risk_notes)
...
def test_low_conviction():
    ...
    # Verify: Should be rejected
    assert decision.approved == False, f"Expected approved=False, got {decision.approved}"
    assert "Conviction" in decision.reason, f"Expected 'Conviction' in reason, got: {decision.reason}"
    assert decision.recommended_size == 0.0, f"Expected recommended_size=0.0, got {decision.recommended_size}"
    
    # Verify: Conviction check failed
    conviction_check_failed = any("Conviction" in note and "✗" in note for note in decision.risk_notes)
...
def test_insufficient_edge():
    ...
    # Verify: Should be rejected
    assert decision.approved == False, f"Expected approved=False, got {decision.approved}"
    assert "Edge" in decision.reason, f"Expected 'Edge' in reason, got: {decision.reason}"
    assert decision.recommended_size == 0.0, f"Expected recommended_size=0.0, got {decision.recommended_size}"
    
    # Verify: Edge check failed
    edge_check_failed = any("Edge" in note and "✗" in note for note in decision.risk_notes)
