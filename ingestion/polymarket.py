...
def fetch_markets(limit: int = 100, active_only: bool = True) -> List[Market]:
    ...
    try:
        ...
        record_event(
            event_type="api_error",
            details={"source": "polymarket", "error": error_msg, "type": type(e).__name__},
            severity="error"
        )
        return []


def categorize_from_question(question: str) -> str:
    """
    Infer category from question text when API doesn't provide category.
    """
    if not question:
        return "unknown"
    ...
    if any(kw in q_lower for kw in [
        ...
        "white house", "primary", "vote", "supreme court"
    ]):
        return "politics"
    ...
    if any(kw in q_lower for kw in [
        ...
        "nato", "war", "invasion", "ceasefire", "sanctions", "putin",
        "xi jinping", "invasion", "conflict"
    ]):
        return "geopolitics"
    ...
    if any(kw in q_lower for kw in [
        ...
        "defi", "nft", "solana", "cardano", "dogecoin"
    ]):
        return "crypto"
    
    # Sports
    if any(kw in q_lower for kw in [
        "nba", "nfl", "nhl", "mlb", "world cup", "super bowl",
        "finals", "championship", "playoff", "lakers", "celtics",
        "cowboys", "patriots", "messi", "lebron", "soccer", "football"
    ]):
        return "sports"
    
    # Weather
    if any(kw in q_lower for kw in [
        "hurricane", "tornado", "earthquake", "temperature", "snow",
        "rain", "drought", "flood", "climate", "degrees", "celsius", "fahrenheit"
    ]):
        return "weather"
    
    # Tech/Pop Culture
    if any(kw in q_lower for kw in [
        "gta", "rihanna", "album", "movie", "album", "ai", "tech",
        "chatgpt", "elon", "tesla", "spacex"
    ]):
        return "pop-culture"
...
def normalize_category(raw_category: str, question: str = "") -> str:
    """
    Normalize Polymarket category names to agent-expected categories.
    If no category provided, infer from question text.
    
    Args:
        raw_category: Category from API (may be None)
        question: Question text to infer from if no category
    """
    # If we have a category from API, normalize it
    if raw_category and raw_category != "unknown":
        cat_lower = raw_category.lower().replace("-", " ").replace("_", " ")
        
        # Politics
        if any(kw in cat_lower for kw in ["politic", "election", "congress", "president", "us current"]):
            return "politics"
        
        # Geopolitics
        if any(kw in cat_lower for kw in ["geopolitic", "international", "world", "war", "conflict"]):
            return "geopolitics"
        
        # Crypto
        if any(kw in cat_lower for kw in ["crypto", "bitcoin", "ethereum", "blockchain", "defi"]):
            return "crypto"
        
        # Sports
        if any(kw in cat_lower for kw in ["sport", "nba", "nfl", "soccer", "football", "baseball", "hockey"]):
            return "sports"
        
        # Weather
        if any(kw in cat_lower for kw in ["weather", "climate", "hurricane", "temperature"]):
            return "weather"
        
        # Tech
        if any(kw in cat_lower for kw in ["tech", "ai", "technology", "software"]):
            return "tech"
        
        return raw_category  # Keep original if no match
    
    # No category from API - infer from question
    return categorize_from_question(question)
...
def parse_polymarket_market(data: Dict[str, Any]) -> Optional[Market]:
    ...
    try:
        ...
        raw_category = data.get("category") or data.get("tag")
        category = normalize_category(raw_category, question)
        ...
        # Extract volume
        volume_24h = float(data.get("volume") or data.get("volume24hr") or data.get("volume_24h") or 0.0)
        ...
        liquidity_score = float(data.get("liquidityScore") or data.get("liquidity") or 0.0)
