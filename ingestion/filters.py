...
def filter_by_volume_range(markets: List[Market], min_volume: float, max_volume: float = float('inf')) -> List[Market]:
    """
    ...
    filtered = [
        m for m in markets 
        if min_volume <= m.volume_24h <= max_volume
    ]
    
    print(f"📊 Filtered to {len(filtered)} markets with volume ${min_volume:,.0f}-${max_volume:,.0f}")
...
def test_filtering():
    ...
    for m in markets:
        ...
        print(f"    Volume: ${m.volume_24h:,.0f} | Liquidity: {m.liquidity_score:.1f} | " +
    ...
    if len(tradeable) == expected_tradeable:
        print(f"✅ TEST PASSED: {len(tradeable)} tradeable markets (expected {expected_tradeable})")
        return True
    else:
        print(f"❌ TEST FAILED: {len(tradeable)} tradeable markets (expected {expected_tradeable})")
        return False
