...
class GeopoliticalAgent(BaseAgent):
    ...
    def update_theses(self) -> List[Thesis]:
        ...
        try:
            ...
            print(f"📊 Fetched {len(markets)} geopolitical markets (volume > ${self.min_volume:,.0f})")
            ...
            for market in markets:
                try:
                    ...
                    if thesis and thesis.edge > self.min_edge:
                        ...
                        print(f"✅ Thesis generated: {market.question[:60]}... (edge: {thesis.edge:.2%})")
                    else:
                        print(f"⏭️  Skipped: {market.question[:60]}... (no signal or low edge)")
    ...
    def generate_thesis(self, market: Market, news_events: List = None) -> Thesis:
        ...
        thesis_text = (
            f"Market underpriced based on {matching_events} recent events: {event_headlines}"
        )
    ...
    def _get_event_headlines(self, market: Market, news_events: List, max_length: int = 100) -> str:
        """
...
def test_geopolitical_agent():
    ...
    if thesis_none is None:
        ...
        print(f"   ⚠️ Generated thesis for non-matching market (edge: {thesis_none.edge:.2%})")
